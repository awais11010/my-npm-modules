...
/**
 * js npm re-port [minus threejs functions] by bonsai3d
 *
...
/**
 * js port by dspdog
 * -- not including all-zero-valued "control" characters block
 *
 * License: Public Domain
 *
 **/
...
var BitmapFont = function(){
    ...
    var fullArr = [
        font8x8_basic_128, //offset 0
        font8x8_ext_latin_96, //offset 128 =0+128
        font8x8_greek_58, //offset 224 =128+96
        font8x8_misc_10, //offset 282 =224+58
        font8x8_box_128, //offset 292 =282+10
        font8x8_block_32, //offset 420 =292+128
        font8x8_hiragana_96, //offset 452 =420+32
        font8x8_sga_26 //offset 544 =452+92
    ];

    var font8x8_allChars = [].concat.apply([], fullArr);

    this.font8x8_allChars = font8x8_allChars;

    this.validLettersStr = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~¡¢£¤¥¦§¨©ª«¬®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏÐÑÒÓÔÕÖ×ØÙÚÛÜÝÞßàáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿΐΑΒΓΔΕΖΗΘΙΚΛΜΝΞΟΠΡ΢ΣΤΥΦΧΨΩΪΫάέήίΰαβγδεζηθικλμνξοπρςστυφχψω─━│┃┄┅┆┇┈┉┊┋┌┍┎┏┐┑┒┓└┕┖┗┘┙┚┛├┝┞┟┠┡┢┣┤┥┦┧┨┩┪┫┬┭┮┯┰┱┲┳┴┵┶┷┸┹┺┻┼┽┾┿╀╁╂╃╄╅╆╇╈╉╊╋╌╍╎╏═║╒╓╔╕╖╗╘╙╚╛╜╝╞╟╠╡╢╣╤╥╦╧╨╩╪╫╬╭╮╯╰╱╲╳╴╵╶╷╸╹╺╻╼╽╾╿▀▁▂▃▄▅▆▇█▉▊▋▌▍▎▏▐░▒▓▔▕▖▗▘▙▚▛▜▝▞▟ぁあぃいぅうぇえぉおかがきぎくぐけげこごさざしじすずせぜそぞただちぢっつづてでとどなにぬねのはばぱひびぴふぶぷへべぺほぼぽまみむめもゃやゅゆょよらりるれろゎわゐゑをんゔ゛゜ゝゞ";
};

BitmapFont.prototype.totalPtsInString = function(str){
    var total=0;
    for(var i=0; i<str.length; i++){
        var c=str.charAt(i);
        if(this.pointTotalsPerLetterObj[c])total+=this.pointTotalsPerLetterObj[c];
    }
    return total;
};


function shiftSetOfPts(sop,vec){
    return sop.map(function(p){
        return [p[0]+vec[0],p[1]+vec[1],p[2]+vec[2]];
    });
}

BitmapFont.prototype.ptsForString = function(str, _lineLen){
    var scope = this;
    var xPerLetter = 10.0;
    var yPerLetter = 10.0;
    var lineLen = _lineLen || 32.0;

    var pts = [];

    for(var i=0; i<str.length; i++){
        var char = str.charAt(i);
        var xs = (i%lineLen)*xPerLetter;
        var ys = Math.floor(i/lineLen)*-yPerLetter;
        pts.push(scope.getPtArr_byUnicodeLetter_shifted(char,[xs,ys,0.0]));
    }

    pts = [].concat.apply([], pts);
    //var geom = setOfPts2Geom(pts);

    return pts;
}


BitmapFont.prototype.ptsPerLetterForString = function(str, _lineLen){
    var scope = this;
    var xPerLetter = 10.0;
    var yPerLetter = 10.0;
    var lineLen = _lineLen || 32.0;
    ...
    for(var i=0; i<str.length; i++){
        ...
        var xs = (i%lineLen)*xPerLetter;
        var ys = Math.floor(i/lineLen)*-yPerLetter;
        bitmapPtsArrs.push(scope.getBitmapPts_byUnicodeLetter_shifted(char,[xs,ys,0.0]))
        //pts.push(scope.getPtArr_byUnicodeLetter_shifted(char,[xs,ys,0.0]));
    }
    ...
    return bitmapPtsArrs;
}


/*
getBitmapPts_byUnicodeLetter_shifted
 */

BitmapFont.prototype.randomValidLetter = function(){
    return this.validLettersStr.charAt(Math.random()*this.validLettersStr.length-1);
}

BitmapFont.prototype.getValidLettersStr = function(){
    var validLetters = [];

    for(var i=0; i<999999; i++){
        var testChar = String.fromCharCode(i);
        if(this.isValidLetter(testChar)){
            validLetters.push(testChar);
        }
    }

    var validLettersStr = _.uniq(validLetters).join("");

    console.log(validLettersStr, validLettersStr.length);
    return validLettersStr;
}

BitmapFont.prototype.getIndexForLetter = function(letter){
    var charCode = letter.charCodeAt(0);
    var offset = 0;

    if(charCode>=0 && charCode<=0x007F){
        //normal letters, do nothing...
    }else if(charCode>=0x00A0 && charCode<=0x00FF){ //latin 96
        offset = 128;
        charCode = charCode-0x00A0;
    }else if(charCode>=0x0390 && charCode<=0x03C9){ //greek 58
        offset = 224;
        charCode = charCode-0x0390;
    }else if(charCode>=0x2500 && charCode<=0x257F){ //box 128
        offset = 292;
        charCode = charCode-0x2500;
    }else if(charCode>=0x2580 && charCode<=0x259F){ //block 32
        offset = 420;
        charCode = charCode-0x2580;
    }else if(charCode>=0x3040 && charCode<=0x309F){ //hiragana 96
        offset = 452;
        charCode = charCode-0x3040;
    }else if(charCode>=0xE541 && charCode<=0xE55A){ //hiragana 96
        offset = 544;
        charCode = charCode-0xE541;
    }else{
        //if not found we render character #0 which is empty
        offset = -1;
        charCode = 1;
    }
...
BitmapFont.prototype.getBitmap_byIndex = function(index){
    index = clamp(index,0,this.font8x8_allChars.length-1);
    var bitMap = this.font8x8_allChars[index];
    var mask;
    var fullMap=[];
    var thisLine=[];
    for (var x=0; x < 8; x++) {
        ...
        for (var y=0; y < 8; y++) {
            mask = bitMap[x] & 1 << y;
            thisLine.push(mask ? 1 : 0);
        }
        fullMap.push(thisLine.slice(0)); //slice creates shallow copy from (start, end [last])
    }
    return fullMap;
};


BitmapFont.prototype.getBitmapPts_byIndex = function(index){
    index = clamp(index,0,this.font8x8_allChars.length-1);
    var bitMap = this.font8x8_allChars[index];
    var pts=[];
    for (var x=0; x < 8; x++) {
        for (var y=0; y < 8; y++) {
            if(bitMap[x] & 1 << y)pts.push([x,y,0]);
        }
    }
    return pts;
};

BitmapFont.prototype.getBitmapPts_byIndex_shifted = function(index, shift){
    index = clamp(index,0,this.font8x8_allChars.length-1);
    var bitMap = this.font8x8_allChars[index];
    var pts=[];
    ...
    /*for (var x=0; x < 8; x++) {
        for (var y=0; y < 8; y++) {
            if(bitMap[x] & 1 << y)pts.push([x+shift[0],y+shift[1],shift[2]]);
        }
    }*/
    ...
    while(x--){
        y=7;
        while(y--){
            if(bitMap[x] & 1 << y)pts.push([y+shift[0],-x+shift[1],shift[2]]);
        }
    }
...
BitmapFont.prototype.getBitmapPts_byUnicodeLetter = function(letter){ //returns arr[8][8] of 1's and 0's
    return this.getBitmapPts_byIndex(this.getIndexForLetter(letter));
};

BitmapFont.prototype.getBitmapPts_byUnicodeLetter_shifted = function(letter, shift){ //returns arr[8][8] of 1's and 0's
    return this.getBitmapPts_byIndex_shifted(this.getIndexForLetter(letter), shift);
};

BitmapFont.prototype.getBitmap_byUnicodeLetter = function(letter){ //returns arr[8][8] of 1's and 0's
    return this.getBitmap_byIndex(this.getIndexForLetter(letter));
};

BitmapFont.prototype.getBitmapStr_byUnicodeLetter = function(letter){ //prints char to console as X's and spaces
    var charStrArrs = this.getBitmap_byIndex(this.getIndexForLetter(letter)).map(function(bitArr){return bitArr.map(function(bit){return bit?"X":" ";})});
    charStrArrs = charStrArrs.map(function(strArr){return strArr.join("");});
    var charStr = charStrArrs.join("\n");
    return charStr;
};

BitmapFont.prototype.getBitmapStr_byIndex = function(index){ //prints char to console as X's and spaces
    var charStr = this.getBitmap_byIndex(index).join("\n").map(function(bit){return bit?"X":" ";});
    return charStr;
};

BitmapFont.prototype.getBitmapArr_byString = function(str){
    var scope = this;
    return str.map(function(letter){
        return scope.getBitmap_byUnicodeLetter(letter);
    });
};

BitmapFont.prototype.getBitmapStrArr_byString = function(str){
    var scope = this;
    return str.split("").map(function(letter){
        return scope.getBitmapStr_byUnicodeLetter(letter);
    });
};

BitmapFont.prototype.getBitmapStringFromString_vertical = function(str){ //vertical print-out
    var scope = this;
    return str.split("").map(function(letter){
        return scope.getBitmapStr_byUnicodeLetter(letter);
    }).join('\n');
...
BitmapFont.prototype.getBitmapStringFromString_horizontal = function(str, scrollMin, scrollMax){ //horizontal print-out
    var scope = this;
    var smin = scrollMin||0;
    var smax = scrollMax||99999;
    var letters_rows = str.split("").map(function(letter){
        return scope.getBitmapStr_byUnicodeLetter(letter).split('\n');
    });
    ...
    for(var i=0;i<8;i++){
        str+=pullRow(i).substring(smin,smax)+"\n";
    }

    return str;
};

BitmapFont.prototype.getPtArrPerLetter_noShift = function(str){
    var scope = this;
    return str.split("").map(function(letter){
        return scope.getPtArr_byUnicodeLetter(letter);
    });
};

BitmapFont.prototype.getPtArr_byUnicodeLetter = function(letter){
    return this.bitmapStr2Pts(this.getBitmapStr_byUnicodeLetter(letter));
};

BitmapFont.prototype.getPtArr_byUnicodeLetter_shifted = function(letter, shift){
    //return this.getBitmapPts_byUnicodeLetter_shifted(letter, shift);
    return this.getBitmapPts_byIndex_shifted(this.getIndexForLetter(letter), shift); //inlined version
};

BitmapFont.prototype.isValidLetter = function(letter){
    return this.getBitmapStr_byUnicodeLetter(letter).indexOf("X")!=-1;
};

//alias name:
//BitmapFont.prototype.string2Bitmaps = BitmapFont.prototype.getBitmapArr_byString;
//BitmapFont.prototype.string2Pts = BitmapFont.prototype.getPtArr_byString;

BitmapFont.prototype.bitmapStr2Pts = function(str){
    var pts = [];
    var rows = str.split("\n");
    rows.forEach(function(row,y){
        var cols = row.split("");
        cols.forEach(function(letter,x){
            if(letter=='X'){
                pts.push([x,8-y,0]);
            }
        });
...
BitmapFont.prototype.bitmap2Pts_shifted = function(bitmap,shift){
    var pts = [];
    var rows = bitmap;
    rows.forEach(function(row,y){
        var cols = row;
        cols.forEach(function(letter,x){
            if(letter){
                pts.push([x+shift[0],8-y+shift[1],shift[2]]);
            }
        });
    });
    return pts;
...
/*
BitmapFont.prototype.bitmapStr2Pts_shifted = function(str,shift){
    var pts = [];
    var rows = str.split("\n");
    rows.forEach(function(row,y){
        var cols = row.split("");
